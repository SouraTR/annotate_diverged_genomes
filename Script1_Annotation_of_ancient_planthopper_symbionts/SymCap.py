...
work_dir = "/scratch/soura/genomecomp2/01_genome_annotation/Piotr/Vidania_annos/"

  ### Folder with genomes to annotate. Any fasta files in that dir will be regarded as genomes for annotation
genomes_for_annotation_dir = "/scratch/soura/genomecomp2/01_genome_annotation/Piotr/Vidania_genomes/"

  ### Folder containing alignments of all protein-coding genes to be annotated. File names: gene_prot.fasta

protein_ref_dir = "/scratch/soura/genomecomp2/01_genome_annotation/Piotr/Planthopper-ancient-symbioses---supplementary-materials/Script1_Annotation_of_ancient_planthopper_symbionts/SymCap_files/Vidania_protein_alignments/"

  ### Folder containing alignments of all rRNA, tmRNA and ncRNA genes to be searched. File names: gene.fasta
rRNA_ref_dir = "/scratch/soura/genomecomp2/01_genome_annotation/Piotr/Planthopper-ancient-symbioses---supplementary-materials/Script1_Annotation_of_ancient_planthopper_symbionts/SymCap_files/Vidania_RNA_references/"

  ### Table with detailed information about every gene: name, EC number, etc.
gene_detail_table = "/scratch/soura/genomecomp2/01_genome_annotation/Piotr/Planthopper-ancient-symbioses---supplementary-materials/Script1_Annotation_of_ancient_planthopper_symbionts/SymCap_files/Vidania_gene_list_final.txt"
...
Blocks_to_run = ["B1", "B2", "B3", "B4", "B5", "B6"]
# Blocks_to_run = ["B1", "B2", "B3"]
...
Compl = {'A': 'T', 'G': 'C', 'T': 'A', 'C': 'G', 'N':'N', 'Y': 'R', 'R': 'Y', 'K': 'M', 'M': 'K'}
...
def ImportFastaRefs(fasta_file):
   ...
   return(Seq_list)
		...
		# Imports a multifasta as a list of lists, where seRcondary lists consist of two
...
if "B3" in Blocks_to_run:
    ...
    for filename in glob.glob("%s/*_prot.fasta" % protein_ref_dir):
        ...
        if not glob.glob("%s/%s_prot.hmm" % (protein_ref_dir, gene_name)):
            print("hmmbuild --singlemx --amino -o temp %s/%s_prot.hmm %s%s_prot.fasta" % (protein_ref_dir, gene_name, protein_ref_dir, gene_name))
...
if "B5" in Blocks_to_run:
    ...
    for i in range(0,len(genome_list)):                  ### i - genome coords in genome_list; in Prot_seq_list, this needs to be +2'ed
        ...
        ORFs_used = {} ### This dictionary contains assignments of all ORFs - and will hopefully help avoid naming the same gene twice!
